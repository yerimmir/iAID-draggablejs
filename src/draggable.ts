// 받아오는 이벤트의 타입 정의
type EventType = MouseEvent | PointerEvent | TouchEvent

// 이벤트 핸들러의 타입
type EventHandler<T> = (event?: T) => void;
// 이벤트 처리 함수들의 타입
type Handler = (event : EventType) => EventType;
...
// unbind(이벤트 영역, "이벤트", 이벤트함수) => 이벤트 리스너 삭제
...
export class Draggable {
    constructor({ press = noop, drag = noop, release = noop, mouseOnly = true }) {
        ...
        this._mousedown = (e) => {
            if (e.which > 1 || this._ignoreMouse === true) {
                return;
            };
    ...
    private _unbind(): void {
        ...
        if (!this._mouseOnly) {
            ...
            unbind(this.document, "touchmove", this._touchmove);
            unbind(this.document, "touchend", this._touchend);
        }
    }
    ...
    bindTo(_element): void {
        ...
        if (this._element) {
            this._unbind();
            console.log("bindTo" + this._element);
        }
    ...
    update({ press, drag, release, mouseOnly }): void {
        this._pressHandler = proxy(normalizeEvent, press);
        this._dragHandler = proxy(normalizeEvent, drag);
        this._releaseHandler = proxy(normalizeEvent, release);
        this._mouseOnly = mouseOnly;
    ...
    // 변수 선언
    private _pressHandler: Handler;
    private _dragHandler: Handler;
    private _releaseHandler: Handler;
    private _mouseOnly: Boolean;
    private _ignoreMouse: Boolean;
    private _touchstart: EventHandler<TouchEvent>;
    private _touchmove: EventHandler<TouchEvent>;
    private _touchend: EventHandler<TouchEvent>;
    private _restoremouse: () => void;
    private _mousedown: EventHandler<MouseEvent>;
    private _mousemove: EventHandler<MouseEvent>;
    private _mouseup: EventHandler<MouseEvent>;
    private _pointerdown: EventHandler<PointerEvent>;
    private _pointermove: EventHandler<PointerEvent>;
    private _pointerup: EventHandler<PointerEvent>;
    private _element: HTMLElement | null; // 드래그 이벤트를 listening(확인하는) 할 DOM element...?
}